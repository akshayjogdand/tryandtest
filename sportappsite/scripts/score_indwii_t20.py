...
# Batsmen
# Lokesh Rahul
batsman_two = Player.objects.get(id=112)

# Virat Kohli
batsman_one = Player.objects.get(id=106)

# Bowlers
# Sheldon Cottrell, Deepak Chahar, Kesrick Williams, Khary Pierre
bowler_one = (
    Player.objects.get(id=430),
    Player.objects.get(id=127),
    Player.objects.get(id=584),
    Player.objects.get(id=489),
)

# Mohammed Shami, Washington Sundar, Hayden Walsh Jnr., Ravindra Jadeja, Bhuvneshwar Kumar, Yuzvendra Chahal, Kuldeep Yadav, Kieron Pollard
bowler_two = (
    Player.objects.get(id=37),
    Player.objects.get(id=266),
    Player.objects.get(id=587),
    Player.objects.get(id=173),
    Player.objects.get(id=154),
    Player.objects.get(id=402),
    Player.objects.get(id=54),
    Player.objects.get(id=16),
)

# Most Valuable Players
# Kohli
mvp_one = Player.objects.get(id=106)

# Lokesh
mvp_two = Player.objects.get(id=112)
...
def margin(prediction):
    ...
    if (
        ...
        and prediction.win_series_margin == 1
    ):
        tps = TouramentPredictionScore()
        tps.prediction = prediction
...
def run(fake_match_id, *args):
    ...
    tournament = Tournament.objects.get(id=15)
...
"""
import datetime
t = Tournament.objects.get(id=15)
v = Venue.objects.get(id=1)
m = Match()
md = '12/12/2019'

m.tournament = t
m.match_number = 4
m.name = 'Ind vs. Wiind T20, Final Scores: pre-Tournament Predictions + Match Predictions'
m.short_display_name = 'Ind vs. Wiind T-20, Final Scores'
m.fake_match = True
