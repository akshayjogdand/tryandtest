...
def setup_teams(tournament, j):
    ...
    group_one = (j.team_one, j.team_two)
    group_two = (j.team_three, j.team_four)

    return winning_teams_by_rank, winning_teams_by_team, finalists, group_one, group_two
...
def pot(prediction, pot):
    points = 0
    players = prediction.pot_vars().values()
    # Player of the Tournament

    if pot in players:
        tps = TouramentPredictionScore()
        tps.prediction = prediction
        tps.rule = "Picked Player of the Tournament (+1500)"
        points = points + 1500
        tps.points = 1500
        tps.player = pot
...
def teams(
    prediction,
    winning_teams_by_rank,
    winning_teams_by_team,
    finalists,
    group_one,
    group_two,
):
    ...
    group_one_picks = (prediction.top_team_one, prediction.top_team_two)
    group_two_picks = (prediction.top_team_three, prediction.top_team_four)

    # 2a 1
    # picked any team in G1
    tps = TouramentPredictionScore()
    tps.prediction = prediction
    tps.rule = "Picked any Team from Group 1 (+500)"
    to_add = []
    for t in group_one_picks:
        if t in group_one:
            tps.points = tps.points + 500
            points = points + 500
            to_add.append(t)
    if len(to_add) > 0:
        add_teams(tps, to_add)
        tps.save()

    # 2a 1
    # picked any team in G2
    tps = TouramentPredictionScore()
    tps.prediction = prediction
    tps.rule = "Picked any Team from Group 2 (+500)"
    to_add = []
    for t in group_two_picks:
        if t in group_two:
            tps.points = tps.points + 500
            points = points + 500
            to_add.append(t)
    ...
    # 2b 1
    # picked any in G1 in order
    tps = TouramentPredictionScore()
    tps.prediction = prediction
    tps.rule = "Picked any Team from Group 1, in order (+500)"
    to_add = []
    if group_one_picks[0] == group_one[0]:
        to_add.append(group_one[0])
        tps.points = tps.points + 500
        points = points + 500
    if group_one_picks[1] == group_one[1]:
        to_add.append(group_one[1])
        tps.points = tps.points + 500
        points = points + 500
    if len(to_add) > 0:
        add_teams(tps, to_add)
    ...
    # 2b 2
    # picked any in G2 in order
    tps = TouramentPredictionScore()
    tps.prediction = prediction
    tps.rule = "Picked any Team from Group 2, in order (+500)"
    to_add = []
    if group_two_picks[0] == group_two[0]:
        to_add.append(group_two[0])
        tps.points = tps.points + 500
        points = points + 500
    if group_two_picks[1] == group_two[1]:
        to_add.append(group_two[1])
        tps.points = tps.points + 500
        points = points + 500
    if len(to_add) > 0:
        add_teams(tps, to_add)
    ...
    tps = TouramentPredictionScore()
    tps.prediction = prediction
    tps.rule = "Picked both Teams from Group 1 (+1000)"
    to_add = []
    if group_one_picks[0] in group_one and group_one_picks[1] in group_one:
        to_add.append(group_one_picks)
        tps.points = tps.points + 1000
        points = points + 1000
        tps.save()
    ...
    tps = TouramentPredictionScore()
    tps.prediction = prediction
    tps.rule = "Picked both Teams from Group 2 (+1000)"
    to_add = []
    if group_two_picks[0] in group_two and group_two_picks[1] in group_two:
        to_add.append(group_two_picks)
        tps.points = tps.points + 1000
        points = points + 1000
        tps.save()

    # 2d 1
    # Both Teams in G1 in order
    tps = TouramentPredictionScore()
    tps.prediction = prediction
    tps.rule = "Picked both Teams from Group 1, in order (+1000)"
    to_add = []
    if group_one_picks[0] == group_one[0] and group_one_picks[1] in group_one:
        to_add.append(group_one_picks)
        tps.points = tps.points + 1000
        points = points + 1000
        tps.save()
    ...
    # 2d 2
    # Both Teams in G2 in order
    tps = TouramentPredictionScore()
    tps.prediction = prediction
    tps.rule = "Picked both Teams from Group 2, in order (+1000)"
    to_add = []
    if group_two_picks[0] == group_two[0] and group_two_picks[1] in group_two:
        to_add.append(group_two_picks)
        tps.points = tps.points + 1000
        points = points + 1000
        tps.save()
    ...
    def handle(self, tournament_id, **kwargs):
        ...
        for member_group in tournament.participating_member_groups.all():
            ...
            for prediction in MemberTournamentPrediction.objects.filter(
                ...
                points = points + pot(prediction, j.player_of_the_tournament)
