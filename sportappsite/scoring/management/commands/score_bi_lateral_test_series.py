...
def get_fake_match(tournament):
    try:
        m = Match.objects.get(
            fake_match=True, tournament=tournament, match_type=Match.TEST
        )
    except Match.DoesNotExist:
        m = Match()
        m.tournament = tournament
        final_match = (
            tournament.matches.filter(match_type=Match.TEST)
            .order_by("-match_date")
            .first()
        )
        m.fake_match = True
...
class Command(BaseCommand):
    help = "Score a Bi-lateral Test Series"
    ...
    def handle(self, tournament_id, **kwargs):
        ...
        j = ScoreSeries.objects.get(tournament=tournament, series=Match.TEST)
        ...
        # clear old scores
        logger.info(f"Clearing old Test Series final Leaderboards for {tournament}")
        for tps in TouramentPredictionScore.objects.filter(
            prediction__tournament=tournament, prediction__prediction_format=Match.TEST,
        ).all():
            tps.delete()
