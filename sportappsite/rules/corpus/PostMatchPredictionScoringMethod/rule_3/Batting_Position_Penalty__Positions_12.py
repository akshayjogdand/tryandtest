        ...
        if bp is not None:
            if bp.position >= start_position and bp.position <= end_position:
                ...
                if skip_penalty == False:
                    ...
                    if bp.player == prediction.super_player:
                        to_penalize.append(bp.player)

    for player in to_penalize:
        score = prediction_players[player]
        if score is not None:
            total_penalty = total_penalty + abs(score)
...
calculation = (
    ...
    else (total_penalty_points(prediction, 1, 2))
)
